...
#####################  모듈들 ########################
import random
from art import logo # art.py 모듈을 사용하여 로고를 출력합니다.

# deal_card() 함수를 만들어 카드 리스트중 랜덤으로 하나 리턴한다.
def deal_card():
    '''랜덤으로 카드를 뽑는 함수'''
    cards = [11, 2, 3, 4, 5, 6, 7, 8, 9, 10, 10, 10, 10]
    card = random.choice(cards) # 리스트에서 랜덤으로 하나를 고른다.
    return card
...
def play_game():
    ...
    # 블랙잭? 21초과? 한장 더?
    while not is_game_over: # 게임종료가 아닐때 계속 반복
        user_score = calculate_score(user_cards)
        computer_score = calculate_score(com_cards)
        print(f' 당신의 카드: {user_cards}, 현재 점수: {user_score}')
        print(f' 컴퓨터의 카드: {com_cards[0]}\n')
        # 블랙잭 확인과 21 초과이면 게임 종료
        if user_score == 0 or computer_score == 0 or user_score > 21:
            ...
            if user_should_deal == "y":
                user_cards.append(deal_card()) # 카드를 한장 더 받는다. 
            else: # 더이상 카드를 받지 않으면 반복문을 종료한다.
                is_game_over = True
    ...
    print(f" 컴퓨터 덱은: {com_cards}, 최종 점수: {computer_score}")
...
def calculate_score(cards):
    """입력한 카드들의 더한값을 리턴"""
    if sum(cards) == 21 and len(cards) == 2: # 입력한 카드리스트의 합이 21이고 카드가 2장일때
        return 0
    if 11 in cards and sum(cards) > 21: # 입력한 카드리스트에 11이 있고 합이 21보다 클때
        cards.remove(11) # 11을 제거한다.
        cards.append(1) # 1을 추가한다.
    return sum(cards)
